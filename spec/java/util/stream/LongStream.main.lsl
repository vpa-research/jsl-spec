...
: LongStreamLSL
{
    ...
    @Phantom proc _dropWhile_loop (i: int, dropLength: int, predicate: LongPredicate): void
    {
        if (action CALL(predicate, [this.storage[i]]))
            dropLength += 1;
        else
    ...
    @Phantom proc _takeWhile_loop (i: int, takeLength: int, predicate: LongPredicate): void
    {
        if (action CALL(predicate, [this.storage[i]]))
            takeLength += 1;
        else
